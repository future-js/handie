import {
  isFunction, isString, isPlainObject,
  includes, each, hasOwnProp, mixin, clone } from '../common/helper';
import { setDefaults, getDefaults } from '../storage/helper';
import { post as ajaxPost, put as ajaxPut } from '../http';
import { jsonify, h5f } from '../form';
import { getDefaultTable, refreshTable } from '../table/helper';

import DIALOG_DEFAULTS from './defaults';
...
function resolveSubmitHandler( $dlg, opts ) {
  ...
  return function( evt, jsonified, inst, submitEvt ) {
    ...
    req(url, jsonified, () => {
      ...
      $dlg.modal('hide');
    }, mixin({isJson: getDefaults('ajax.RESTful') === true, $waiting: $dlg}, ajaxOpts));
  }
}
 ...
 * opts 的结构为：
 * {
 *   $button: $(getDefault('dialog.button')),
 *   $table: null,
 *   $form: null,
 *   ajax: {
 *     url: "",
...
export function init( $dlg, opts ) {
  const defaultSelector = getDefaults('dialog.selector');
  ...
  const onFormSubmit = resolveSubmitHandler($dlg, opts);
...
export function levelUp( $dlg ) {
  const $backdrop = $dlg.data('bs.modal').$backdrop;
  const increase = dialogLevel * 2 * 10;
...
// 获取最顶级的对话框
export function top() {
  return [].sort.call($('.modal:visible'), ( a, b ) => $(a).css('z-index') * 1 < $(b).css('z-index') * 1).first();
}
