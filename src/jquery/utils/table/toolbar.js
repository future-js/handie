import {
  isString, isFunction, isArray, isPlainObject, isNumeric,
  includes, each, map, hasOwnProp, mixin, clone,
...
import { refreshTable } from './helper';
...
function resolveToolbarActionDefaults( opts ) {
  return mixin({}, getDefaults('table.toolbarActions.basic'), opts);
}
...
function constructCreateButton( createOpts, tableOpts ) {
  ...
  const btnOpts = mixin(true, {}, resolveToolbarActionDefaults(getDefaults('table.toolbarActions.create')), createOpts.button, {
...
function constructBatchButton( batchOpts, tableOpts, $table, $toolbar ) {
  ...
  const btnOpts = mixin(true, {}, resolveToolbarActionDefaults(getDefaults('table.toolbarActions.batch')), {
...
function constructOtherButtons( actions, tableOpts, $table, $toolbar ) {
  ...
  const bindHandler = ( action, $table, $toolbar ) => $toolbar.on('click', `.js-${action.action}`, function( evt ) {
...
function initToolbarLeftGroup( toolbarOpts, tableOpts, $table, $toolbar ) {
  ...
  $table.on({
    'check.bs.table \
     ...
     check-all.bs.table \
     uncheck-all.bs.table': () => $batch.prop('disabled', isEmptyRowData($table)),
    'post-body.bs.table': ( evt, data ) => $batch.prop('disabled', (data.length === 0 || isEmptyRowData($table))),
    'refresh.bs.table': () => {
      if ( tableOpts.url || (tableOpts.ajaxOptions && tableOpts.ajaxOptions.url) ) {
        $batch.prop('disabled', true);
      }
    },
    'page-change.bs.table': () => $batch.prop('disabled', tableOpts.sidePagination === 'server' ? true : isEmptyRowData($table))
  });
...
function constructTableFilter( filterOpts, tableOpts ) {
  ...
  filterOpts = mixin(true, {}, getDefaults('table.toolbarActions.search.filter'), filterOpts);
...
function constructTableSearch( searchOpts, tableOpts ) {
  ...
  if ( selectableFields.length > 1 ) {
    html.push(
      ...
      map(selectableFields, function( field ) {
  ...
  html.push(
    ...
    map(hiddenFields, function( field ) {
...
function calculateTableSearchWidth( searchOpts, $search ) {
  ...
  const defaultSearchWidth = getDefaults('table.toolbarActions.search.width');
...
function initTableSearch( searchOpts, tableOpts, $table, $toolbar ) {
  ...
  // 查询/筛选表单提交事件
  $('form', $search.add($filter)).on('submit', () => {
  ...
  if ( $filter ) {
    ...
    if ( $filter.hasClass('Table-filter--dropdown') ) {
      ...
      $('.js-executeTableFilters', $filter).on('click', () => {
    ...
    // 重置复杂条件查询
    $('.js-resetTableFilters', $filter).on('click', () => {
      $filterForm.trigger('reset');
    });
  }
...
function constructColumnToggleButton( tableOpts ) {
  ...
  each(tableOpts.__tableInst.columns, function( col, idx ) {
    if ( !col.radio && !col.checkbox &&
        !(tableOpts.cardView && !col.cardVisible) &&
        col.switchable && col.field !== getDefaults('table.operationColumn.field') ) {
      const attrs = [
        'type="checkbox"',
