...
import { isString, isFunction, isArray, isPlainObject, toArray, arrayEach, arrayMap, arrayFilter, hasOwnProp, mixin, clone, generateRandomId, initBootstrapTooltip } from "../common/helper";
...
import { refreshTable, isOperationColumnSticky } from "./helper";
...
function constructOperationColumn( col, opts ) {
  ...
  if ( isFunction(opts.formatter) ) {
    ...
    arrayEach(actions, function( btn ) {
    ...
    formatter = function( val, row, idx ) {
      ...
      return generateAction(arrayMap(btns, function( btn ) {
        btn = clone(btn);
...
function resolveOperationColumn( tableOpts ) {
  ...
  const defaultOpts = clone(defaults.table.operationColumn);
  const newCols = [];
...
function resolveTableClass( slim ) {
  let classes = defaults.table.classes;
...
function resolveColumnOptions( cols, showRowNumber ) {
  ...
  return arrayMap(temp, function( col ) {
    const viewDetailOpts = col.viewDetail;
    const rawFormatter = col.formatter;
    ...
    if ( isPlainObject(viewDetailOpts) ) {
      col.formatter = function( val ) {
        if ( isFunction(rawFormatter) ) {
          val = rawFormatter.apply(this, toArray(arguments));
        }
      ...
      col.events = mixin({}, col.events, {
        "click .js-openDetailDialog": function( evt, val, row ) {
...
function resolveTableOptions( opts ) {
  ...
  opts.slim = hasOwnProp("slim", opts) ? opts.slim === true : defaults.table.slim;
  ...
  if ( opts.slim ) {
    ...
    // 支持列宽度控制
    opts.columns = arrayMap(opts.columns, function( col ) {
      if ( hasOwnProp("width", col) ) {
        ...
        col.formatter = function( val ) {
          ...
          if ( isFunction(rawFormatter) ) {
            content = rawFormatter.apply(this, toArray(arguments));
          }
...
function constructStickyColumn( evt ) {
  ...
  const columnField = defaults.table.operationColumn.field;
  ...
  if ( $(`.${cls}`, $tableBody).length === 0 ) {
    const classes = arrayFilter(tableInst.options.classes.split(" "), ( c ) => {
