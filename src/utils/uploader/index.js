import { isFunction, isPlainObject, arrayEach, hasOwnProp, objectKeys, mixin } from "../common/helper";
...
function stashQiniuUploaderEvents( evts ) {
  ...
  if ( evts ) {
    arrayEach(["FilesAdded", "BeforeUpload", "FileUploaded"], function( name ) {
...
export function image( $btn, opts ) {
  ...
  $btn = $($btn);
  opts = mixin(true, {
    limit: defaults.uploader.limit,
    draggable: defaults.uploader.draggable,
  ...
  if ( isPlainObject(opts.settings) ) {
    const defaultSettings = {
        ...
        filters: {
          mime_types: [{
            title: "图片文件",
            extensions: defaults.uploader.extension.image
          }]
        }
      };
