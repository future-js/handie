...
function hasOwnProp$1(prop, obj) {
...
function isArrayLike(target) {
  ...
  if (isObject(target) && !isGlobal(target)) {
    ...
    if (target.nodeType === 1 && length || !isArray$1(target) && !isFunction$1(target) && (length === 0 || isNumber(length) && length > 0 && length - 1 in target)) {
      result = true;
    }
  }
...
function isPlainObject$1(target) {
  ...
  try {
    if (target.constructor && !hasOwnProp$1("constructor", target) && !hasOwnProp$1("isPrototypeOf", target.constructor.prototype)) {
      return false;
    }
  } catch (err) {
    return false;
  ...
  return key === undefined || hasOwnProp$1(key, target);
}
...
function treatAsArray(target) {
  return isString$1(target) || isArray$1(target) || isArrayLike(target);
}
...
function clone(target) {
  return isArray(target) ? [].concat(target) : isPlainObject(target) ? mixin(true, {}, target) : target;
}
...
function setData(key, val) {
  ...
  eachItem(keys, function (k) {
    if (!hasOwnProp$1(k, tmp)) {
      tmp[k] = {};
    }
...
function alertMessage(message, settings) {
  ...
  if (isString$1(settings)) {
    type = settings;
  } else if (isPlainObject$1(settings)) {
    type = settings.type;
...
function setDefaults(settings) {
  extendTarget(true, defaults, settings);
}
...
var defaults$2 = {
  ajax: {
    ...
    errorHandler: function errorHandler(evt, req, settings, err) {
      ...
      if (code >= 500) {
        alertMessage(defaults$2.ajax.serverErrorText);
      } else if (code >= 400) {
        ...
        // 支持 {"message": ""} 形式的错误信息
        if (isPlainObject$1(resJson) && hasOwnProp$1("message", resJson)) {
...
function change($sel, val, callback) {
  ...
  if (val == null || val === "") {
    ...
    opts = isArray$1(val) ? map(val, function (v) {
...
function jsonifyFormData($form, callback) {
  ...
  arrayEach(isArray$1($form) ? $form : isQueryStr($form) ? queryStr2SerializedArr($form) : $($form).serializeArray(), function (p) {
    jsonData[p.name] = hasOwnProp$1(p.name, jsonData) ? [].concat(jsonData[p.name], p.value) : p.value;
  });

  if (isFunction$1(callback)) {
    var newJson = callback(jsonData);

    if (isPlainObject$1(newJson)) {
      jsonData = newJson;
    }
  }
...
function serialize($form, filter$$1) {
  ...
  if (!isFunction$1(filter$$1)) {
    filter$$1 = defaults$2.form.filter;
  }

  if (!isFunction$1(serializer)) {
    serializer = defaults$2.form.serializer;
  }
...
function resolveAjaxResult(res, callback, settings) {
  ...
  if (!hasOwnProp$1("alertType", settings)) {
    settings.alertType = "system";
  }

  defaults$2.ajax.responseHandler(res, callback, settings);
}

function waitingForResponse($target) {
  var text = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaults$2.ajax.waitingText;
...
function httpReq(url, method, params, callback, isJson, settings) {
  ...
  params = isPlainObject$1(params) || isArray$1(params) ? params : jsonifyFormData(params);
...
function generateAjaxUtil(method) {
  return function (url, params, callback, isJson) {
    ...
    if (isPlainObject$1(url)) {
       ...
       * })
       */
      else if (isPlainObject$1(callback)) {
         ...
         * })
         */
        else if (isPlainObject$1(isJson)) {
...
function resolveSubmitHandler($dlg, opts) {
  ...
  return function (evt, jsonified, inst, submitEvt) {
    ...
    req(url, jsonified, function () {
      ...
      $dlg.modal("hide");
    }, extendTarget({ isJson: defaults$2.ajax.RESTful === true, $waiting: $dlg }, ajaxOpts));
  };
}
 ...
 * }
 */
function init($dlg, opts) {
  var defaultSelector = defaults$2.dialog.selector;

  if (isPlainObject$1($dlg)) {
    opts = $dlg;
    $dlg = $(defaultSelector);
  } else {
    $dlg = $($dlg);
  ...
  if ($dlg.is(defaultSelector)) {
    if (!hasOwnProp$1("$button", opts) && !hasOwnProp$1("$btn", opts)) {
      opts.$button = $(defaults$2.dialog.button);
    }

    if (!hasOwnProp$1("$table", opts)) {
...
function refillImageItem($img, url) {
  ...
  if ($item.length > 0) {
    ...
    if (url) {
      ...
      if (ext && !includes(ext.toLowerCase(), defaults$2.uploader.extension.image.split(","))) {
...
function resolveDefaultTriggerOptions() {
  return extendTarget(true, {}, builtInActions, defaults$2.generator.actions);
}
...
function resolveTriggerOptionsSimply(opts) {
  if (isString$1(opts) && hasOwnProp$1(opts, resolveDefaultTriggerOptions())) {
    opts = { action: opts };
  }
...
function constructActionButton(opts, settings) {
  ...
  var hasChildren = hasOwnProp$1("actions", opts) && isArray$1(opts.actions);

  var btnCls = clone(opts.classes);

  if (isString$1(btnCls)) {
    btnCls = btnCls.split(" ");
  } else if (!isArray$1(btnCls)) {
    btnCls = [];
  }
...
function generateImageItem(url, alt, removable) {
  ...
  if (url && ext && !includes(ext.toLowerCase(), defaults$2.uploader.extension.image.split(","))) {
...
function insertImageItem(opts) {
  ...
  if (isNaN(columnCount) || columnCount <= 0) {
    columnCount = defaults$2.generator.imageColumnCount;
  }
...
function action(actions, wrapped) {
  ...
  actions = actions.concat(defaults$2.table.rowActions);
...
function resolveGeoCoordinate(coord, defaultName) {
  if (isPlainObject$1(coord) && isString$1(coord.longitude) && isString$1(coord.latitude)) {
...
function resolveRoutePlan(from, to, map$$1) {
  if (arguments.length === 1) {
    if (!isPlainObject$1(from) || !hasOwnProp$1("from", from) || !hasOwnProp$1("to", from)) {
      return "";
    }

    if (hasOwnProp$1("map", from)) {
      map$$1 = from.map;
    }
  ...
  if (!isPlainObject$1(from) || !isPlainObject$1(to)) {
    return "";
  }
...
function supportWebSocket() {
  return hasOwnProp$1("WebSocket", window);
}

function supportWebNotification() {
  return hasOwnProp$1("Notification", window);
}

function supportLocalStorage() {
  return hasOwnProp$1("localStorage", window);
}
...
function showNoticeLayer(text, type, forceOver) {
  var opts = defaults$2.notification.layer;
...
function show(opts) {
  ...
  if (!(supportWebNotification() && isPlainObject$1(opts) && hasOwnProp$1("title", opts))) {
    return;
  }
...
function init$1(opts) {
  ...
  if (!(supportWebSocket() && isPlainObject$1(opts) && hasOwnProp$1("url", opts))) {
    return;
  }
...
$.fn.bootstrapTable.Constructor.prototype.initToolbar = function () {
  ...
  if (!isPlainObject$1(toolbarOpts) && !tableOpts.showColumns) {
    return;
  }
...
function resolveToolbarActionDefaults(opts) {
  return extendTarget({}, defaults.table.toolbarActions.basic, opts);
}
...
function constructCreateButton(createOpts, tableOpts) {
  var hasDlgOpts = hasOwnProp$1("dialog", createOpts);
  ...
  if (!(isPlainObject$1(createOpts) && ($.isEmptyObject(createOpts) || hasOwnProp$1("button", createOpts) || hasDlgOpts))) {
    return "";
  }

  var btnOpts = extendTarget(true, {}, resolveToolbarActionDefaults(defaults.table.toolbarActions.create), createOpts.button, {
...
function constructBatchButton(batchOpts, tableOpts, $table, $toolbar) {
  ...
  if (isArray$1(batchOpts)) {
    actions = batchOpts;
  } else if (isPlainObject$1(batchOpts) && isArray$1(batchOpts.actions)) {
  ...
  var btnOpts = extendTarget(true, {}, resolveToolbarActionDefaults(defaults.table.toolbarActions.batch), {
  ...
  if (actions.length === 1 && !isArray$1(actions[0])) {
...
function initToolbarLeftGroup(toolbarOpts, tableOpts, $table, $toolbar) {
  if (!isPlainObject$1(toolbarOpts)) {
    return;
  }
  ...
  if (tableOpts.__$dialog) {
    init(tableOpts.__$dialog, extendTarget({ $table: $table }, resolveCreateOptions(toolbarOpts).dialog));
  }
...
function constructTableFilter(filterOpts, tableOpts) {
  if (!isPlainObject$1(filterOpts)) {
    filterOpts = { selector: filterOpts };
  }

  filterOpts = extendTarget(true, {}, defaults.table.toolbarActions.search.filter, filterOpts);
...
function constructTableSearch(searchOpts, tableOpts) {
  ...
  html.push("<input id=\"" + searchId + "\" class=\"form-control\" type=\"text\" name=\"" + defaultField.name + "\" value=\"\" placeholder=\"" + (defaultField.placeholder || "") + "\">", map(hiddenFields, function (field) {
    return "<input type=\"hidden\" name=\"" + field.name + "\" value=\"" + (hasOwnProp$1("value", field) ? field.value : "") + "\">";
...
function calculateTableSearchWidth(searchOpts, $search) {
  ...
  var defaultSearchWidth = defaults.table.toolbarActions.search.width;
...
function initTableSearch(searchOpts, tableOpts, $table, $toolbar) {
  if (!isPlainObject$1(searchOpts) || $(".Table-search", $toolbar).length > 0) {
    return;
  }
...
function constructColumnToggleButton(tableOpts) {
  ...
  eachItem(tableOpts.__tableInst.columns, function (col, idx) {
    if (!col.radio && !col.checkbox && !(tableOpts.cardView && !col.cardVisible) && col.switchable && col.field !== defaults.table.operationColumn.field) {
...
function constructOperationColumn(col, opts) {
  ...
  } else if (isArray$1(actions) && actions.length > 0) {
    var btns = [];

    eachItem(actions, function (btn) {
      btn = resolveTriggerOptionsSimply(btn);

      if (isPlainObject$1(btn)) {
...
function resolveOperationColumn(tableOpts) {
  ...
  var defaultOpts = clone(defaults.table.operationColumn);
  var newCols = [];
  ...
  if (!hasCheckboxOpt && isPlainObject$1(tableOpts.toolbar) && hasOwnProp$1("batch", tableOpts.toolbar)) {
...
function resolveTableClass(slim) {
  var classes = defaults.table.classes;
...
function resolveColumnOptions(cols, showRowNumber) {
  var temp = isArray$1(cols) ? cols.concat([]) : [];
  ...
  return map(temp, function (col) {
    ...
    // 打开查看详情对话框
    if (isPlainObject$1(viewDetailOpts)) {
...
function resolveTableOptions(opts) {
  ...
  opts.slim = hasOwnProp$1("slim", opts) ? opts.slim === true : defaults.table.slim;
  opts.classes = resolveTableClass(opts.slim);

  if (isPlainObject$1(opts.operation) && !hasOwnProp$1("sticky", opts.operation)) {
    opts.operation.sticky = opts.slim ? true : defaults.table.operationColumn.sticky;
  }

  // 「瘦身」的数据表格
  if (opts.slim) {
    // 没指定 `sticky` 配置项则默认开启
    if (!hasOwnProp$1("sticky", opts)) {
      opts.sticky = true;
    }

    // 支持列宽度控制
    opts.columns = map(opts.columns, function (col) {
      if (hasOwnProp$1("width", col)) {
        ...
        col.formatter = function (val) {
          ...
          if (content == null) {
            content = hasOwnProp$1("undefinedText", opts) ? opts.undefinedText : $.fn.bootstrapTable.defaults.undefinedText;
          }
...
function constructStickyColumn(evt) {
  ...
  var columnField = defaults.table.operationColumn.field;
...
function init$2($table, opts) {
  ...
  if (sticky) {
    ...
    } else if (isPlainObject$1(sticky)) {
      stickyOpts = extendTarget(true, defaultStickyOpts, sticky);
    }

    if (isPlainObject$1(stickyOpts)) {
...
function image($btn, opts) {
  ...
  opts = extendTarget(true, {
    limit: defaults$2.uploader.limit,
    draggable: defaults$2.uploader.draggable,
    // Settings for Plupload
    settings: null
  }, opts);

  if (isPlainObject$1(opts.settings)) {
    var defaultSettings = {
      browse_button: $btn.get(0),
      filters: {
        mime_types: [{
          title: "图片文件",
          extensions: defaults$2.uploader.extension.image
        }]
      }
    };
    ...
    if (hasOwnProp$1("__muu", uploader)) {
...
function init$3(opts) {
  opts = extendTarget(true, {}, defaults$3, opts);
...
function watermark(opts) {
  ...
  opts = extendTarget(true, {}, defaults$2.watermark, { id: generateRandomId("watermark") }, opts);
...
window.muu = {
  ...
  dialog: dialog, table: table, form: form, field: field, select: select,
